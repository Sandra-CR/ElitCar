...
if(isset($_SESSION['role']) && $_SESSION['role'] == Role::ADMIN->value) {
    ...
    if (isset ($_GET['id'])) {
        ...
        $sql2 = "SELECT * FROM address_particulier WHERE id_user='$id'"; 
...
?>
    ...
    <h1 class="text-center my-4"><?= "$user[first_name] $user[last_name]" ?></h1>
        ...
        <div class="row">
            <div class="col-6">
                <div class="text-center">
                    <img src="<?= htmlentities($user['pp']) ?>" class="image-fluid rounded mt-5"  width="50%" alt="">
                </div>
            </div>
            <div class="col-6">
                <ul class="list-group mt-5">
                    <li class="list-group-item text-center">Email : <?= htmlentities($user['mail']) ?></li>
                    <li class="list-group-item text-center">Date de Naissance : <?= htmlentities($user['birthdate']) ?></li>
                    <li class="list-group-item text-center">Numéro de teléphone : <?= htmlentities($user['num_mobile']) ?></li>
                    <?php $roles= ["Visiteur","Client", "Locataire", "Agence", "Administrateur"]; ?>
                ...
                <div class="container-btn-read">
                    <a class="btn btn-success my-2" href="view/admin/particular_table">Revenir au Tableau</a>
                    <a class="btn btn-warning" href="view/admin/update_admin_particular?id=<?= $id ?>"">Modifier La fiche de l'utilisateur</a>
                </div>
            </div> 
        </div>
    </div>
