...
if (!empty($quartier) || !empty($ville) || !empty($region) || !empty($codepostale)) {
    try {
        ...
        $insertStmt = $pdo->prepare("INSERT INTO address_professional (street_name, neighborhood, city, region, postal_code, additional_description, id_pro) VALUES (:street_name, :neighborhood, :city, :region, :postal_code, :additional_description, :id_pro)");
        ...
        $insertStmt->bindParam(':id_pro', $id);
        // Exécution de la requête
        $insertStmt->execute(); // Exécute la requête d'insertion des données dans la base de données
        sendMessage("Adresse postale ajoutee avec succes", "success", "../../view/professional/adress_professional"); // Affiche un message de succès après l'insertion des données

    } catch (PDOException $e) {
        error_log("Erreur PDO: " . $e->getMessage(), 0); // Enregistre l'erreur PDO dans le journal des erreurs
        sendMessage("Une erreur s'est produite lors de l'ajout de l'adresse postale. Veuillez reessayer plus tard.", "failed", "../../view/professional/adress_professional"); // Affiche un message d'erreur en cas d'échec de l'insertion des données
    } finally {
        $pdo = null; // Ferme la connexion à la base de données
    }
}else {
    sendMessage("Veuillez remplir tous les champs du formulaire.", "failed", "../../view/professional/adress_professional"); // Affiche un message d'erreur si l'un des champs du formulaire est vide
}
?>
